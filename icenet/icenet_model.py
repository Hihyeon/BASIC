...
class IceNet(nn.Module):
	...
	def __init__(self):
		...
		self.relu = nn.ReLU(inplace=True)  # inplace하면 input으로 들어온 것 자체를 수정하겠다는 뜻. 메모리 usage가 좀 좋아짐. 하지만 input을 없앰.
		self.e_conv1 = nn.Conv2d(2,32,3,1,1,bias=True)  # 모든 output channel = 32
	...
	def forward(self, y, maps, e, lowlight):
		...
		# AGEB(adaptive gamma estimation block)(gamma map)
		global x_r
		...
		# gamma correction
		global enhanced_Y
		enhanced_Y = torch.pow(y, x_r)  # y^(x_r)
		# color restoration
